...
def gen_batch_function(data_folder, image_shape):
    ...
    def get_batches_fn(batch_size): 
        ...
        for batch_i in range(0, len(image_paths), batch_size):#Onece we go in this loop this generate on batch of trainig data
            images = []
            mask_images = []
            for image_file in image_paths[batch_i:batch_i+batch_size]:
                mask_image_file = label_paths[os.path.basename(image_file)]

                image = scipy.misc.imresize(scipy.misc.imread(image_file), image_shape) #(160, 576,3) image reduced to single colour
                #here the image label means a mask
                mask_image = scipy.misc.imresize(scipy.misc.imread(mask_image_file), image_shape) #(160, 576, 3)

                mask_bg = np.all(mask_image == background_color, axis=2) #we can first extract the backgroud red pixels
                mask_bg = mask_bg.reshape(*mask_bg.shape, 1)#reshaping this to (160,576,1)
                mask_image = np.concatenate((mask_bg, np.invert(mask_bg)), axis=2)#now there are two types of pixels in the label (foreground(road) and background)
